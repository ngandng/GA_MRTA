...
nc = 2*round(pc*nPop/2);  % Number of Offsprings (also Parnets)...
                      % also, number of new candidate solutions are generated by crossover in each generation
...
%     TournamentSize = 3;   % Tournamnet Size: group for competing to find out winner to choose as parent
% end

pause(0.01); % Due to a bug in older versions of MATLAB

% Feasibilization = @(x, y) Feasibilization(x, y, beta);
%% Initialization
...
for it = 1:MaxIt
    ...
    for k = 1:nc/2
        ...
        % Apply Crossover
        % disp("Applying crossover for agents " + i1 +" and " + i2);
